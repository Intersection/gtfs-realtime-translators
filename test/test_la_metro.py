...
def test_la_data_with_floats(la_metro_rail):
    la_metro_mutated = copy.deepcopy(la_metro_rail)
    la_metro_mutated['items'][0]['minutes'] = 8.0

    with pendulum.test(pendulum.datetime(2019,2,20,17,0,0)):
        translator = LaMetroGtfsRealtimeTranslator(la_metro_mutated, stop_id='80122')
